...
num_train = size_R2D2(1)+size_R2D2(2);
num_eval = size_R2D2(3);
idx_R2D2 = [1:num_train];
...
for i=1:numel(alex_idx)
  fprintf('[%i] %s %f\n', i, list_R2D2{alex_idx(i)+num_train}, alex_score(i));
end

disp('Vgg19 によるランキング');
for i=1:numel(vgg_idx)
  fprintf('[%i] %s %f\n', i, list_R2D2{vgg_idx(i)+num_train}, vgg_score(i));
end

disp('Densenet によるランキング');
for i=1:numel(dense_idx)
  fprintf('[%i] %s %f\n', i, list_R2D2{dense_idx(i)+num_train}, dense_score(i));
end